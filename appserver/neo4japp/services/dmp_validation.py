...
Loads the vendored JSON Schema 1.2 (neo4japp/schemas/formats/madmp_v1_2.json,
see docs/dmp-schema/README.md and docs/field-mapping.md) and validates payloads
on create/update, turning schema errors into a flat list of actionable per-field
error dicts.

The schema ships inside the ``neo4japp`` package rather than under docs/ because
the appserver image is built with appserver/ as its context -- anything at the
repository root is absent at runtime.
"""
...
from neo4japp.schemas import formats

DMP_SCHEMA_RESOURCE = 'madmp_v1_2.json'
...
def _load_validator():
    ...
    with _lock:
        if _validate_fn is None:
            with resources.open_text(formats, DMP_SCHEMA_RESOURCE) as f:
                schema = json.load(f)
            _validate_fn = fastjsonschema.compile(schema, formats=_CUSTOM_FORMATS)
    return _validate_fn


class DMPValidationError(ValueError):
    """
