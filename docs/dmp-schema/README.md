...
## Contents
  ...
  (schema lives at appserver/neo4japp/schemas/formats/madmp_v1_2.json,
   inside the package so it is present in the appserver image)
...
## Using in application code (Python / marshmallow-flask appserver)
  ...
  with open("../../appserver/neo4japp/schemas/formats/madmp_v1_2.json") as f:
      SCHEMA = json.load(f)
